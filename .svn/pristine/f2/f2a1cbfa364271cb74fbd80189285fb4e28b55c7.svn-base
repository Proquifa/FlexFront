package mx.com.proquifa.proquifanet.rsl.vista.vistas.catalogos.proveedores.zitemRenderer.empresasGeneral
{
		...
		override public function measure():void{
			...
			if(count > 0){
				...
				if(parent != null && parent.hasOwnProperty("currentCubosPorFila"))
					numCubosXfilaFromParent = (parent as PanelContactosAnimado).currentCubosPorFila;
			}
		}
			...
			if($containerWidth >0 ){
				...
				for (var i:int = 0; i < count; i++) {
					...
					if((currentElement as Object).hasOwnProperty("puntoMedio")){
						...
						(currentElement as CuadradoContacto).puntoMedio = (_x + (anchoParaItem/2))-5;
					}
