package mx.com.proquifa.proquifanet.rsl.vista.vistas.componentes.pq
{
	...
	public class MobViewStack extends Group
	{
		...
		private var _activateFade:Boolean = false;
		...
		protected function fadeMostrar( event:UIComponent ):void
		{
			fadeIn( event ); 
		}
		
		protected function fadeOcultar( event:UIComponent ):void
		{
			fadeOut( event ); 
		}
		protected function fadeIn( $target:Object ):void
		{
			...
			fade.target = $target;
			fade.alphaFrom = ($target as UIComponent).alpha;
			fade.duration = 500;
			fade.alphaTo = 1;
			fade.play();
		}
		...
		public function get activateFade():Boolean
		{
			return _activateFade;
		}
		
		public function set activateFade(value:Boolean):void
		{
			_activateFade = value;
		}
