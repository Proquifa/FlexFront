package vista.modelo
{
	...
	public class ModeloEmbalarProductos extends EventDispatcher
	{
		...
		private var cambioEstadoPedidoAPorColectar: Boolean;
		public function recibeCambiaEstadoPedidoAPorColectar(result:Boolean):void{
			cambioEstadoPedidoAPorColectar=result;
			dispatchEvent(new Event('enviarCambiaEstadoPedidoAPorColectarModeloEmbalarProductos'))
		}
		
		[Bindable (event="enviarCambiaEstadoPedidoAPorColectarModeloEmbalarProductos")]
		public function get enviarCambiaEstadoPedidoAPorColectar():Boolean{
			return cambioEstadoPedidoAPorColectar;
		}		
		
		private var cambioEstadoPedidoAEmbalado: Boolean;
		public function recibeCambiaEstadoPedidoAEmbalado(result:Boolean):void{
			cambioEstadoPedidoAEmbalado=result;
			dispatchEvent(new Event('enviarCambiaEstadoPedidoAEmbaladoModeloEmbalarProductos'))
		}
		
		[Bindable (event="enviarCambiaEstadoPedidoAEmbaladoModeloEmbalarProductos")]
		public function get enviarCambiaEstadoPedidoAEmbalado():Boolean{
			return cambioEstadoPedidoAEmbalado;
		}		
		
		private var cambioEstadoPedidoAPorEmbalar: Boolean;
		public function recibeCambiaEstadoPedidoAPorEmbalar(result:Boolean):void{
			cambioEstadoPedidoAPorEmbalar=result;
			dispatchEvent(new Event('enviarCambiaEstadoPedidoAPorEmbalarModeloEmbalarProductos'))
		}
		
		[Bindable (event="enviarCambiaEstadoPedidoAPorEmbalarModeloEmbalarProductos")]
		public function get enviarCambiaEstadoPedidoAPorEmbalar():Boolean{
			return cambioEstadoPedidoAPorEmbalar;
		}		
		...
		public function recibePartidasINSP(part:ArrayCollection):void{
			...
			if(part == null)
				...
				_sql = new Query(lstPartidas,["nombreCliente","prioridad","ruta"],true);
				punterosCliente = _sql.getPunteros(["nombreCliente", null, null],"nombreCliente");
				...
				for each (var clienteA:String in punterosCliente) 
				{
					...
					var punterosPorRuta:Array = _sql.getPunteros([clienteA,null,"ruta"], "ruta");
					
					for(var y = 0; y < punterosPorRuta.length; y++){
						...
						var contP2:Number = 0;
						var contP3:Number = 0;
						punterosTemp = _sql.getPunteros([clienteA,null, punterosPorRuta[y]]);
