package mx.com.proquifa.proquifanet.rsl.vista.vistas.comun.coverflowpqnet.clases
{
	...
	public class CoverComoUnGrupoGris extends Group
	{
		...
		private var myParent:CoverFlowPQNetGris; 
		...
		public function CoverComoUnGrupoGris()
		{
			super();
		...
		private function coverComoUnGrupo_ADDEDEvent(event:Event):void
		{
			...
			if(this.parent.parent is CoverFlowPQNetGris)
			{
				myParent = (this.parent.parent as CoverFlowPQNetGris);
			}
		}
		...
		private function showBotones_EffectEndHandler(e:EffectEvent):void
		{
			...
			agregarListeners();
			if(vista=="oferta")
				cambiarNivelEnOferta();
			if(vista=="clasificacion")
				cambiarNivelEnClasificacion();
		}
		...
		private function cambiarNivelEnOferta():void
		{
			var cambiarNivel:EventoCatProOferta = new EventoCatProOferta(EventoCatProOferta.CAMBIA_NIVEL_EN_VISTA_OFERTA);
			cambiarNivel.nivel = myParent.currentNivel;
			cambiarNivel.configuracionPrecio = _currentFamilia;
			dispatchEvent(cambiarNivel);
		}
		private function cambiarNivelEnClasificacion():void
		{
			var cambiarNivel:EventoCatProClasificacion = new EventoCatProClasificacion(EventoCatProClasificacion.CAMBIA_NIVEL_EN_VISTA_CLASIFICACION);
			cambiarNivel.posicionDeLaConfEnElCinturon = _currentFamilia.numFila;
			cambiarNivel.configuracionPrecio = _currentFamilia;
			dispatchEvent(cambiarNivel);
		}
