package mx.com.proquifa.proquifanet.rsl.vista.vistas.catalogos.clientes.itemRenderer.empresasGeneral
{
	...
	public class LayoutFilaBottonPanelContactos extends LayoutBase
	{
		...
		override public function measure():void{
			...
			if(count > 0){
				parent = ((((target as Group).automationOwner as ItemRendererPanelContactosAnimado).automationOwner as List).automationOwner);
				if(parent != null && parent.hasOwnProperty("currentCubosPorFila"))
					numCubosXfilaFromParent = (parent as ClientePanelContactosAnimado).currentCubosPorFila;
			}
		}
		
		public function LayoutFilaBottonPanelContactos()
		{
			super();
		}
			...
			if($containerWidth >0 ){
				...
				for (var i:int = 0; i < count; i++) {
					...
					if((currentElement as Object).hasOwnProperty("puntoMedio")){
						...
						(currentElement as ClienteCuadroContacto).puntoMedio = (_x + (anchoParaItem/2))-5;
					}
