package mx.com.proquifa.vista.modelo
{
	import flash.utils.ByteArray;

	[Bindable]
	[RemoteClass(alias="mx.com.proquifa.proquifanet.modelo.ventas.interlocutor.GestionarAvisoDeCambios")]
	public class GestionarAvisoDeCambios
	{
		
		private var _origen:String;
		private var _concepto:String;
		private var _codigo:String;
		private var _unidad:String;
		private var _fabrica:String;
		private var _cantidad:String;
		private var _cliente:String;
		private var _fee:Date;
		private var _idAvisoCambios:int;
		private var _idPAvisoCambios:int;
		private var _tel:String;
		private var _ext:String;
		private var _tel2:String;
		private var _ext2:String;
		private var _idContacto:int;
		private var _tipo:String;
		private var _contacto:String;
		private var _email:String;
		private var _folioNT:String;
		private var _piezas:int;
		private var _monto:Number;
		private var _ppedidos:int;
		private var _dia1:int;
		private var _dia2:int;
		private var _dia3:int;
		private var _dia4:int;
		private var _urgente:int;
		private var _enTiempo:int;
		private var _fueraDeTiempo:int;
		private var _espera:int; //1 = SI; 0 = NO; 3 = Continuar
		private var _documento:ByteArray;
		private var _fpg:Date;
		private var _fdp:Date;
		private var _fnueva:Date;
		private var _pt:Number;
		private var _pu:Number;
		private var _cpedido:String;
		
		private var _fnuevaMS:Number;
		private var _feeMs:Number;
		private var _fdpMs:Number;
		private var _fpgMs:Number;
		private var _gestionada:Number;
		private var _idPendiente:Number;
		private var _usuario:String;
		private var _idPPedido:Number;
		
		//VISTA
		private var _selecionDato:Boolean;
		private var _fechaContinuar:Date;
		private var _idPCompra:Number;
		private var _comprador:String;
		private var _continuaGestion:Boolean;
		private var _tipoRadio:String;
		private var _tipoFlete:String;
		private var _piezasRegular:int;
		private var _piezasProgramado:int;
		private var _piezasExpress:int;
		private var _piezasFleteNormal:int;
		private var _piezasEntregaUnica:int;
		
		public function GestionarAvisoDeCambios()
		{
		}

		public function get piezasEntregaUnica():int
		{
			return _piezasEntregaUnica;
		}

		public function set piezasEntregaUnica(value:int):void
		{
			_piezasEntregaUnica = value;
		}

		public function get piezasExpress():int
		{
			return _piezasExpress;
		}

		public function set piezasExpress(value:int):void
		{
			_piezasExpress = value;
		}

		public function get piezasFleteNormal():int
		{
			return _piezasFleteNormal;
		}

		public function set piezasFleteNormal(value:int):void
		{
			_piezasFleteNormal = value;
		}

		public function get piezasProgramado():int
		{
			return _piezasProgramado;
		}

		public function set piezasProgramado(value:int):void
		{
			_piezasProgramado = value;
		}

		public function get piezasRegular():int
		{
			return _piezasRegular;
		}

		public function set piezasRegular(value:int):void
		{
			_piezasRegular = value;
		}

		public function get tipoFlete():String
		{
			return _tipoFlete;
		}

		public function set tipoFlete(value:String):void
		{
			_tipoFlete = value;
		}

		public function get tipoRadio():String
		{
			return _tipoRadio;
		}

		public function set tipoRadio(value:String):void
		{
			_tipoRadio = value;
		}

		public function get continuaGestion():Boolean
		{
			return _continuaGestion;
		}

		public function set continuaGestion(value:Boolean):void
		{
			_continuaGestion = value;
		}

		public function get comprador():String
		{
			return _comprador;
		}

		public function set comprador(value:String):void
		{
			_comprador = value;
		}

		public function get idPPedido():Number
		{
			return _idPPedido;
		}

		public function set idPPedido(value:Number):void
		{
			_idPPedido = value;
		}

		public function get origen():String
		{
			return _origen;
		}

		public function set origen(value:String):void
		{
			_origen = value;
		}

		public function get concepto():String
		{
			return _concepto;
		}

		public function set concepto(value:String):void
		{
			_concepto = value;
		}

		public function get idAvisoCambios():int
		{
			return _idAvisoCambios;
		}

		public function set idAvisoCambios(value:int):void
		{
			_idAvisoCambios = value;
		}

		public function get idPAvisoCambios():int
		{
			return _idPAvisoCambios;
		}

		public function set idPAvisoCambios(value:int):void
		{
			_idPAvisoCambios = value;
		}

		public function get tel():String
		{
			return _tel;
		}

		public function set tel(value:String):void
		{
			_tel = value;
		}

		public function get ext():String
		{
			return _ext;
		}

		public function set ext(value:String):void
		{
			_ext = value;
		}

		public function get tel2():String
		{
			return _tel2;
		}

		public function set tel2(value:String):void
		{
			_tel2 = value;
		}

		public function get ext2():String
		{
			return _ext2;
		}

		public function set ext2(value:String):void
		{
			_ext2 = value;
		}

		public function get idContacto():int
		{
			return _idContacto;
		}

		public function set idContacto(value:int):void
		{
			_idContacto = value;
		}


		public function get email():String
		{
			return _email;
		}

		public function set email(value:String):void
		{
			_email = value;
		}

		public function get folioNT():String
		{
			return _folioNT;
		}

		public function set folioNT(value:String):void
		{
			_folioNT = value;
		}

		public function get piezas():int
		{
			return _piezas;
		}

		public function set piezas(value:int):void
		{
			_piezas = value;
		}

		public function get monto():Number
		{
			return _monto;
		}

		public function set monto(value:Number):void
		{
			_monto = value;
		}

		public function get ppedidos():int
		{
			return _ppedidos;
		}

		public function set ppedidos(value:int):void
		{
			_ppedidos = value;
		}

		public function get dia1():int
		{
			return _dia1;
		}

		public function set dia1(value:int):void
		{
			_dia1 = value;
		}

		public function get dia2():int
		{
			return _dia2;
		}

		public function set dia2(value:int):void
		{
			_dia2 = value;
		}

		public function get dia3():int
		{
			return _dia3;
		}

		public function set dia3(value:int):void
		{
			_dia3 = value;
		}

		public function get dia4():int
		{
			return _dia4;
		}

		public function set dia4(value:int):void
		{
			_dia4 = value;
		}

		public function get urgente():int
		{
			return _urgente;
		}

		public function set urgente(value:int):void
		{
			_urgente = value;
		}

		public function get espera():int
		{
			return _espera;
		}

		public function set espera(value:int):void
		{
			_espera = value;
		}

		public function get fee():Date
		{
			return _fee;
		}

		public function set fee(value:Date):void
		{
			_fee = value;
		}

		public function get cliente():String
		{
			return _cliente;
		}

		public function set cliente(value:String):void
		{
			_cliente = value;
		}

		public function get contacto():String
		{
			return _contacto;
		}

		public function set contacto(value:String):void
		{
			_contacto = value;
		}

		public function get enTiempo():int
		{
			return _enTiempo;
		}

		public function set enTiempo(value:int):void
		{
			_enTiempo = value;
		}

		public function get fueraDeTiempo():int
		{
			return _fueraDeTiempo;
		}

		public function set fueraDeTiempo(value:int):void
		{
			_fueraDeTiempo = value;
		}

		public function get tipo():String
		{
			return _tipo;
		}

		public function set tipo(value:String):void
		{
			_tipo = value;
		}

		public function get documento():ByteArray
		{
			return _documento;
		}

		public function set documento(value:ByteArray):void
		{
			_documento = value;
		}

		public function get fpg():Date
		{
			return _fpg;
		}

		public function set fpg(value:Date):void
		{
			_fpg = value;
		}

		public function get fdp():Date
		{
			return _fdp;
		}

		public function set fdp(value:Date):void
		{
			_fdp = value;
		}

		public function get pt():Number
		{
			return _pt;
		}

		public function set pt(value:Number):void
		{
			_pt = value;
		}

		public function get pu():Number
		{
			return _pu;
		}

		public function set pu(value:Number):void
		{
			_pu = value;
		}

		public function get cpedido():String
		{
			return _cpedido;
		}

		public function set cpedido(value:String):void
		{
			_cpedido = value;
		}

		public function get codigo():String
		{
			return _codigo;
		}

		public function set codigo(value:String):void
		{
			_codigo = value;
		}

		public function get unidad():String
		{
			return _unidad;
		}

		public function set unidad(value:String):void
		{
			_unidad = value;
		}

		public function get fabrica():String
		{
			return _fabrica;
		}

		public function set fabrica(value:String):void
		{
			_fabrica = value;
		}

		public function get cantidad():String
		{
			return _cantidad;
		}

		public function set cantidad(value:String):void
		{
			_cantidad = value;
		}

		public function get fnueva():Date
		{
			return _fnueva;
		}

		public function set fnueva(value:Date):void
		{
			_fnueva = value;
		}

		public function get selecionDato():Boolean
		{
			return _selecionDato;
		}

		public function set selecionDato(value:Boolean):void
		{
			_selecionDato = value;
		}

		public function get fnuevaMS():Number
		{
			return _fnuevaMS;
		}

		public function set fnuevaMS(value:Number):void
		{
			_fnuevaMS = value;
		}

		public function get feeMs():Number
		{
			return _feeMs;
		}

		public function set feeMs(value:Number):void
		{
			_feeMs = value;
		}

		public function get fdpMs():Number
		{
			return _fdpMs;
		}

		public function set fdpMs(value:Number):void
		{
			_fdpMs = value;
		}

		public function get fpgMs():Number
		{
			return _fpgMs;
		}

		public function set fpgMs(value:Number):void
		{
			_fpgMs = value;
		}

		public function get gestionada():Number
		{
			return _gestionada;
		}

		public function set gestionada(value:Number):void
		{
			_gestionada = value;
		}

		public function get idPendiente():Number
		{
			return _idPendiente;
		}

		public function set idPendiente(value:Number):void
		{
			_idPendiente = value;
		}

		public function get usuario():String
		{
			return _usuario;
		}

		public function set usuario(value:String):void
		{
			_usuario = value;
		}

		public function get fechaContinuar():Date
		{
			return _fechaContinuar;
		}

		public function set fechaContinuar(value:Date):void
		{
			_fechaContinuar = value;
		}

		public function get idPCompra():Number
		{
			return _idPCompra;
		}

		public function set idPCompra(value:Number):void
		{
			_idPCompra = value;
		}


	}
}