package skins.layouts
{
	...
	public class LayoutListaGrid extends LayoutBase
	{
		public function LayoutListaGrid()
		{
			super();
		}
		 ...
		 *  @private
		 */
		public function set heightMin(value:Number):void
		{
			_heightMin = value;
		}
		
		
		private var _lineaHorizontal:Boolean = true;;
		
		public function get lineaHorizontal():Boolean
		{
			return _lineaHorizontal;
		}
		
		/**
		 *  @private
		 */
		public function set lineaHorizontal(value:Boolean):void
		{
			_lineaHorizontal = value;
		}
		
		
		private var _lineaVertical:Boolean = true;;
		
		public function get lineaVertical():Boolean
		{
			return _lineaVertical;
		}
		
		/**
		 *  @private
		 */
		public function set lineaVertical(value:Boolean):void
		{
			_lineaVertical = value;
		}
		...
		private var _widthMin:Number;
		
		public function get widthMin():Number
		{
			return _widthMin;
		}
		 ...
		 *  @private
		 */
		public function set widthMin(value:Number):void
		{
			_widthMin = value;
		}
		...
		private var _numColumnas:Number;
		
		public function get numColumnas():Number
		{
			return _numColumnas;
		}
		 ...
		 *  @private
		 */
		public function set numColumnas(value:Number):void
		{
			_numColumnas = value;
		}
		...
		public function get numFilas():Number
		{
			return _numFilas;
		}
		 ...
		 *  @private
		 */
		public function set numFilas(value:Number):void
		{
			_numFilas = value;
		}
												...
												cellHeight:int):void
		{
			...
			colors=[0xffffff,0xe9e9e7];   // COLOR HORIZONTAL
			//colors=[0xffffff,0x000000];   // COLOR HORIZONTAL
			alphas=[1,1];
			
			// ESTO ES PARA LAS LINEAS HORIZONTALES
			if(lineaHorizontal)
			{
				var matr:Matrix = new Matrix();
				matr.createGradientBox((widthElement*columnCount)/2, 3,0);
				ratios=[0,100];
				
				this.target.graphics.lineStyle(2);
				this.target.graphics.lineGradientStyle(GradientType.LINEAR,colors,alphas,ratios,matr,SpreadMethod.REFLECT);
				this.target.graphics.moveTo(x, y+heigthElement-1);
				this.target.graphics.lineTo(x+widthElement, y+heigthElement-1);
				this.target.graphics.endFill();
			}
			
			
			// ESTO ES PARA LAS LINEAS VERTICALES
			if(lineaVertical)
			{
				var matHeight:Matrix = new Matrix();
				matHeight.createGradientBox((widthElement),((heigthElement)/2),(Math.PI/2),0,0);
				ratiosH=[0,255];
				
				this.target.graphics.lineStyle(2);
				this.target.graphics.lineGradientStyle(GradientType.LINEAR,colors,[.2,1],ratiosH,matHeight,SpreadMethod.REFLECT);
				this.target.graphics.moveTo(x+widthElement, 0);
				this.target.graphics.lineTo(x+widthElement, y+heigthElement);
				this.target.graphics.endFill();
			}
		}
		...
		//public static const COLOR_GRIS:uint = 0x000000;
		//public static const COLOR_GRIS_SUAVE:uint = 0x000000;
		...
		override public function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			sizesElements(unscaledWidth,unscaledHeight);
		...
		private var widthElement:Number;
		private var heigthElement:Number;
		private function sizesElements(w:Number, h:Number):void
		{
			var totalMinimo:Number=0;
			for (var r:int=0; r < _numColumnas; r++)
			{
				if(_widthMin > 0)
				{
					totalMinimo = _widthMin * (_numColumnas -(r)) ;
					if(totalMinimo < w)
					{
						_requestedColumnCount = (_numColumnas -r);
						break;
					}
				}
				else
				{
					_requestedColumnCount = _numColumnas;
				}
			}
			
			var totalAlto:Number = 0;
			for (var m:int=0; m < _numFilas; m++)
			{
				if(_heightMin > 0)
				{
					totalAlto = _heightMin * (_numFilas - m) ;
					if(totalAlto < h)
					{
						_requestedRowCount = (_numFilas - m);
						break;
					}
					else
					{
				...
				{
					_requestedRowCount = _numFilas;
				}
			}
			
			
			var sum:Number =(_horizontalGap * (_columnCount-1))+ _paddingLeft + paddingRight;
			
			
			explicitColumnWidth = widthElement = (w-sum ) / _requestedColumnCount;
			
			if(_heightMin > 0)
				explicitRowHeight = heigthElement = _heightMin > (h  / _numFilas) ? _heightMin:(h  / _numFilas);
		}
