package skins.skin.graficas
{
	...
	import skins.skin.label.Texto_16_Helvetica;
	
	[Event(name="clickImagen",type="flash.events.MouseEvent")]
	public class PieChartPqGestor extends PieChart
	{
		...
		public function PieChartPqGestor()
		{
			super();
			...
			if (noQuitarOver){
				if( habilitarResumen ){
					...
					if( sItem.item is ResumenConsulta ){
						...
						var stitulo:String = resumen.etiqueta + ' ' + (sItem as ResumenConsulta).totalDocumentos + ' Documentos';
						_vieneRollover = true;
		...
		override public function set dataProvider(value:Object):void
		{
			...
			if( _proposedSerie == _SERIE_NORMAL && sSerieOrigen  != "vs" && value is ArrayCollection )
			{
		...
		private function escucharSeries_UPDATE_COMPLETE_HANDLER(e:FlexEvent):void
		{
			...
			{
				selectedFirst();
				//trace("PieChartPqGestor.escucharSeries_UPDATE_COMPLETE_HANDLER(e)");
			}
		}
		...
		protected function rollOverGrafica(event:ChartItemEvent):void
		{
			...
			var radioLinea:Number = 50; //( (event.currentTarget as PieChartPqGestor).width/5);
			var xOrigen:Number = (event.currentTarget as PieChartPqGestor).width/2;
			var yOrigen:Number = (event.currentTarget as PieChartPqGestor).height/2;
		...
		public var lblDataTip:Texto_16_Helvetica = null;
		...
		override protected function createChildren():void{
			...
			if( lblDataTip == null ){
				lblDataTip = new Texto_16_Helvetica();
		...
		public function aplicarFormatoGestorCompras($lista:ArrayCollection):ArrayCollection{
			_resumenTotal.total = 0;
			_resumenTotal.clientes_proveedores = 0;
			_resumenTotal.totalCompras = 0;
			_resumenTotal.piezasTotal = 0;
