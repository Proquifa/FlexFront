package skins.skin.graficas
{
	...
	public class PieChartPqGris extends PieChart
	{
		...
		private var tituloSinPorcentaje:Boolean = false;
		...
		protected function rollOverGrafica(event:ChartItemEvent):void
		{
			...
			if( habilitarResumen ){
				...
				if( sItem.item is ResumenConsulta ){
					var resumen:ResumenConsulta = sItem.item as ResumenConsulta;
					if(!tituloSinPorcentaje)
					var stitulo:String = resumen.etiqueta + ' ' + sItem.percentValue.toFixed(2) + ' %';
					else
						var stitulo:String = resumen.etiqueta;
					_vieneRollover = true;
					mostrarTotales( resumen, stitulo );
		...
		private function mostrarResumen():void{
			if( habilitarResumen ){
				...
				if (_resumenTotal.totalClientes > 0 ){
					totalClientes = _resumenTotal.totalClientes;
				}
				...
				if( _resumenTotal.totalPedidos > 0 ){
					totalPedidos = _resumenTotal.totalPedidos;
		...
		private function mostrarTotales( $resumen:ResumenConsulta, $titulo:String ):void{
			...
			/*{*/
				...
				/*else{
					description += $resumen.piezasTotal + ' \n';
					description += 'Piezas';
				}
		...
		public function aplicarFormatoConsultas($lista:ArrayCollection, titulo:String = null,etiquetaDiferente:String = null,switchConceptoResultado:Boolean=true, totalCotizacion:int=0, totClientes: int = 0, totPedidos: int = 0 ):ArrayCollection{
			// HABILITADOS
			_resumenTotal.etiquetaEncabezado = titulo == null ?  "Totales" : titulo;
			_resumenTotal.total = 0;
			_resumenTotal.totalClientes = totClientes;
			//_resumenTotal.cotizacionesTotal = totalCotizacion;
			_resumenTotal.partidas = 0;
			_resumenTotal.piezasTotal = 0;
			if(etiquetaDiferente == null)
			_resumenTotal.totalPedidos = totPedidos;
		...
		public function aplicarFormatoConsultasDeDosEtiquetas( $lista:ArrayCollection, titulo:String = null,etiquetaDiferente:String = null, totalClientes: int = 0 ):ArrayCollection{
			// HABILITADOS
			_resumenTotal.etiquetaEncabezado = titulo == null ?  "Totales" : titulo;
			//_resumenTotal.totalDocumentos = 0;
			_resumenTotal.totalClientes = totalClientes;
		...
		public function aplicarFormatoTableroCotNoPed( $lista:ArrayCollection, $totalClientes:Number = 0 ):ArrayCollection{
			// HABILITADOS
			_resumenTotal.total = 0;
			_resumenTotal.totalClientes = $totalClientes;
			_resumenTotal.partidas = 0;
		...
		public function aplicarFormatoInspeccionarProductosPrioridadesEstadisticas( $lista:ArrayCollection, titulo:String = null ):ArrayCollection{
			// HABILITADOS
			_resumenTotal.etiquetaEncabezado = titulo == null ?  "Totales" : titulo;
			_resumenTotal.piezasTotal = 0;
			_resumenTotal.anio = -10;
			lblDataTip.height =30 * factorScale;
			
			return $lista;
		}
		
		public function aplicarFormatoInspeccionarProductosPopxPrioridad( $lista:ArrayCollection, titulo:String = null ):ArrayCollection{
			// HABILITADOS
			_resumenTotal.etiquetaEncabezado = titulo == null ?  "Totales" : titulo;
			_resumenTotal.piezasTotal = 0;
			_resumenTotal.etiqueta = "Piezas"
			tituloSinPorcentaje = true;
			lblDataTip.height =30 * factorScale;
			
			return $lista;
		}
		
		public function aplicarFormatoTableroClientesMarcas( $lista:ArrayCollection, $totalClientes:Number = 0 ):ArrayCollection{
			// HABILITADOS
			_resumenTotal.total = 0;
			_resumenTotal.totalClientes = $totalClientes;
