package mx.com.proquifa.proquifanet.rsl.vista.modelo.comun
{
	import flash.utils.ByteArray;

	[RemoteClass(alias="mx.com.proquifa.proquifanet.modelo.comun.Contacto")]
	[Bindable]
	public class Contacto
	{		
		private var _numFila:Number;
		private var _idContacto:Number;
		private var _idEmpresa:int;
		private var _nombre:String;
		private var _titulo:String;
		private var _puesto:String;
		private var _departamento:String;
		private var _telefono:String;
		private var _telefonoN:String;
		private var _fax:String;		
		private var _EMail:String;
		private var _tipo:String;
		private var _empresa:String;
		private var _habilitado:Boolean;
		private var _nacionalidadEmpresa:String;
		private var _ultimo:String = "";
		private var _direccion:Direccion;
		private var _idEmpleadoEsac:int;
		private var _nombreEsac:String;
		private var _totalContactos:int;
		private var _totalDirecciones:int;
		private var _totalZonas:int;
		private var _agregadoExpo:Boolean;
		private var _habilitadoCliente:Boolean;
		private var _idProveedor:Number;
		private var _extension1:String;
		private var _extension2:String;
		private var _usuarioPConnect:Boolean;
		private var _idFletera:int;
		//Variables agregadas para los nuevos campos que vienen de PConnect
		private var _apellidos:String;
		private var _celular:String;
		private var _antiguedadAnio:int;
		private var _antiguedadMes:int;
		private var _fechaNacimiento:Date;
		
		//se agregan para EV
		private var _industria:String;
		private var _rol:String;
		private var _sector:String;
		private var _nivelIngresos:String;
		
		private var _origenRegistro:String;
		
		private var _totalContactosPorEmpresa:Number;  //esta variable permite un conteo para los items renderes
		private var _totalClientes:Number; //esta variable permite un conteo para los items renderes
		
		//variables agregaada por el Osv para poder utilizarla en el componente  comboCatalogos
		private var _valor:String;
		private var _llave:Number;
		
		//Para cliente
		private var _fechaUltimaActualizacion:Date;
		private var _nivelDecision:String;
		private var _nivelPuesto:String;
		private var _estadoCliente:Boolean
		private var _vieneDE_PopUp:String;
		private var _fk02_Cliente:String;
		private var _idAgenteAduanal:Number;
		private var _mantenimiento:String;
		private var _dificultad:String;
		
		private var _esac:String;
		private var _ev:String;
		
		private var _vinculado:Boolean;
		//VISTA
		private var _nombreCompleto:String;
		private var _deDondeViene:String;
		private var _contactoNAFTA:Boolean;
		private var _firma:ByteArray;
		private var _rutaFirma:String;
		
		private var _foto:ByteArray;
		private var _rutaFoto:String;
		
		
		public function Contacto()
		{
			_nombre = "";
			_apellidos = "";
		}
		
		
		public function get rutaFoto():String
		{
			return _rutaFoto;
		}

		public function set rutaFoto(value:String):void
		{
			_rutaFoto = value;
		}

		public function get foto():ByteArray
		{
			return _foto;
		}

		public function set foto(value:ByteArray):void
		{
			_foto = value;
		}

		public function get rutaFirma():String
		{
			return _rutaFirma;
		}

		public function set rutaFirma(value:String):void
		{
			_rutaFirma = value;
		}

		public function get firma():ByteArray
		{
			return _firma;
		}

		public function set firma(value:ByteArray):void
		{
			_firma = value;
		}

		public function get contactoNAFTA():Boolean
		{
			return _contactoNAFTA;
		}

		public function set contactoNAFTA(value:Boolean):void
		{
			_contactoNAFTA = value;
		}

		public function get idFletera():int
		{
			return _idFletera;
		}

		public function set idFletera(value:int):void
		{
			_idFletera = value;
		}

		public function get deDondeViene():String
		{
			return _deDondeViene;
		}

		public function set deDondeViene(value:String):void
		{
			_deDondeViene = value;
		}

		public function get nombreCompleto():String
		{
			if(apellidos != null && apellidos != "" && apellidos != "ND"){
				_nombreCompleto = nombre +" "+ apellidos; 								
			}
			else{
				_nombreCompleto = nombre;
			}
			return _nombreCompleto;
		}

		public function set nombreCompleto(value:String):void
		{
			_nombreCompleto = value;
		}

		public function get fechaNacimiento():Date
		{
			return _fechaNacimiento;
		}

		public function set fechaNacimiento(value:Date):void
		{
			_fechaNacimiento = value;
		}

		public function get antiguedadMes():int
		{
			return _antiguedadMes;
		}

		public function set antiguedadMes(value:int):void
		{
			_antiguedadMes = value;
		}

		public function get antiguedadAnio():int
		{
			return _antiguedadAnio;
		}

		public function set antiguedadAnio(value:int):void
		{
			_antiguedadAnio = value;
		}

		public function get celular():String
		{
			return _celular;
		}

		public function set celular(value:String):void
		{
			if(value == null || value == ""){
				_celular = "ND";
			}else{
				_celular = value;
			}
		}

		public function get apellidos():String
		{
			return _apellidos;
		}

		public function set apellidos(value:String):void
		{
			if(value == null || value == ""){
				_apellidos = "ND";
			}else{
				_apellidos = value;
			}
		}

		public function get idProveedor():Number
		{
			return _idProveedor;
		}

		public function set idProveedor(value:Number):void
		{
			_idProveedor = value;
		}

		public function get habilitadoCliente():Boolean
		{
			return _habilitadoCliente;
		}
		
		public function set habilitadoCliente(value:Boolean):void
		{
			_habilitadoCliente = value;
		}
		
		public function get fk02_Cliente():String
		{
			return _fk02_Cliente;
		}
		
		public function set fk02_Cliente(value:String):void
		{
			_fk02_Cliente = value;
		}
		
		public function get vieneDE_PopUp():String
		{
			return _vieneDE_PopUp;
		}
		
		public function set vieneDE_PopUp(value:String):void
		{
			_vieneDE_PopUp = value;
		}
		
		public function get estadoCliente():Boolean
		{
			return _estadoCliente;
		}
		
		public function set estadoCliente(value:Boolean):void
		{
			_estadoCliente = value;
		}
		
		public function get agregadoExpo():Boolean
		{
			return _agregadoExpo;
		}
		
		public function set agregadoExpo(value:Boolean):void
		{
			_agregadoExpo = value;
		}
		
		public function get totalZonas():int
		{
			return _totalZonas;
		}
		
		public function set totalZonas(value:int):void
		{
			_totalZonas = value;
		}
		
		public function get totalDirecciones():int
		{
			return _totalDirecciones;
		}
		
		public function set totalDirecciones(value:int):void
		{
			_totalDirecciones = value;
		}
		
		public function get totalContactos():int
		{
			return _totalContactos;
		}
		
		public function set totalContactos(value:int):void
		{
			_totalContactos = value;
		}
		
		public function get nombreEsac():String
		{
			return _nombreEsac;
		}
		
		public function set nombreEsac(value:String):void
		{
			_nombreEsac = value;
		}
		
		public function get idEmpleadoEsac():int
		{
			return _idEmpleadoEsac;
		}
		
		public function set idEmpleadoEsac(value:int):void
		{
			_idEmpleadoEsac = value;
		}
		
		public function get direccion():Direccion
		{
			return _direccion;
		}
		
		public function set direccion(value:Direccion):void
		{
			_direccion = value;
		}
		
		public function get numFila():Number
		{
			return _numFila;
		}
		
		public function set numFila(value:Number):void
		{
			_numFila = value;
		}
		
		public function get totalClientes():Number
		{
			return _totalClientes;
		}
		
		public function set totalClientes(value:Number):void
		{
			_totalClientes = value;
		}
		
		public function get totalContactosPorEmpresa():Number
		{
			return _totalContactosPorEmpresa;
		}
		
		public function set totalContactosPorEmpresa(value:Number):void
		{
			_totalContactosPorEmpresa = value;
		}
		
		public function get nivelIngresos():String
		{
			return _nivelIngresos;
		}
		
		public function set nivelIngresos(value:String):void
		{
			_nivelIngresos = value;
		}
		
		public function get sector():String
		{
			return _sector;
		}
		
		public function set sector(value:String):void
		{
			_sector = value;
		}
		
		public function get rol():String
		{
			return _rol;
		}
		
		public function set rol(value:String):void
		{
			_rol = value;
		}
		
		public function get industria():String
		{
			return _industria;
		}
		
		public function set industria(value:String):void
		{
			_industria = value;
		}
		
		public function get llave():Number
		{
			return _llave;
		}
		
		public function set llave(value:Number):void
		{
			_llave = value;
		}
		
		public function get valor():String
		{
			return _valor;
		}
		
		public function set valor(value:String):void
		{
			_valor = value;
		}
		
		public function get ultimo():String
		{
			return _ultimo;
		}
		
		public function set ultimo(value:String):void
		{
			if(value == null) {
				_ultimo = "";
			}else{
				_ultimo = value;
			}
		}
		
		public function get empresa():String
		{
			return _empresa;
		}
		
		public function set empresa(value:String):void
		{
			if(value == null || value == ""){
				_empresa = "ND";
			}else{
				_empresa = value;	
			}
		}
		
		public function get tipo():String
		{
			return _tipo;
		}
		
		public function set tipo(value:String):void
		{
			if(value == null || value == ""){
				_tipo = "ND";
			}else{
				_tipo = value;	
			}
		}
		
		public function get EMail():String
		{
			return _EMail;
		}
		
		public function set EMail(value:String):void
		{
			if(value == null || value == ""){
				_EMail = "ND";
			}else{
				_EMail = value;	
			}
		}
		
		public function get fax():String
		{
			return _fax;
		}
		
		public function set fax(value:String):void
		{
			if(value == null || value == ""){
				_fax = "ND";
			}else{
				_fax = value;	
			}
		}
		
		public function get telefonoN():String
		{
			return _telefonoN;
		}
		
		public function set telefonoN(value:String):void
		{
			if(value == null || value == ""){
				_telefonoN = "ND";	
			}else{
				_telefonoN = value;
			}
		}
		
		public function get telefono():String
		{
			return _telefono;
		}
		
		public function set telefono(value:String):void
		{
			if(value == null || value == ""){
				_telefono = "ND";	
			}else{
				_telefono = value;
			}
		}
		
		public function get departamento():String
		{
			return _departamento;
		}
		
		public function set departamento(value:String):void
		{
			if(value == null || value == ""){
				_departamento = "ND";	
			}else{
				_departamento = value;
			}
		}
		
		public function get puesto():String
		{
			return _puesto;
		}
		
		public function set puesto(value:String):void
		{
			if(value == null || value == ""){
				_puesto = "PUESTO DESCONOCIDO";	
			}else{
				_puesto = value;
			}
		}
		
		public function get titulo():String
		{
			return _titulo;
		}
		
		public function set titulo(value:String):void
		{
			if(value == null || value == ""){
				_titulo = "ND";	
			}else{
				_titulo = value;
			}
		}
		
		public function get nombre():String
		{
			return _nombre;
		}
		
		public function set nombre(value:String):void
		{
			if(value == null || value == ""){
				_nombre = "ND";	
			}else{
				_nombre = value;
			}
		}
		public function get nacionalidadEmpresa():String
		{
			return _nacionalidadEmpresa;
		}
		
		public function set nacionalidadEmpresa(value:String):void
		{
			if(value == null || value == ""){
				_nacionalidadEmpresa = "ND";	
			}else{
				_nacionalidadEmpresa = value;
			}
		}
		
		public function get habilitado():Boolean
		{
			return _habilitado;
		}
		
		public function set habilitado(value:Boolean):void
		{
			_habilitado = value;	
		}
		
		public function get idContacto():Number
		{
			return _idContacto;
		}
		
		public function set idContacto(value:Number):void
		{
			_idContacto = value;
		}
		
		public function get idEmpresa():int
		{
			return _idEmpresa;
		}
		
		public function set idEmpresa(value:int):void
		{
			_idEmpresa = value;
		}
		
		public function get fechaUltimaActualizacion():Date
		{
			return _fechaUltimaActualizacion;
		}
		
		public function set fechaUltimaActualizacion(value:Date):void
		{
			_fechaUltimaActualizacion = value;
		}
		
		public function get nivelDecision():String
		{
			return _nivelDecision;
		}
		
		public function set nivelDecision(value:String):void
		{
			_nivelDecision = value;
		}
		
		public function get nivelPuesto():String
		{
			return _nivelPuesto;
		}
		
		public function set nivelPuesto(value:String):void
		{
			_nivelPuesto = value;
		}

		public function get origenRegistro():String
		{
			return _origenRegistro;
		}

		public function set origenRegistro(value:String):void
		{
			_origenRegistro = value;
		}

		public function get idAgenteAduanal():Number
		{
			return _idAgenteAduanal;
		}

		public function set idAgenteAduanal(value:Number):void
		{
			_idAgenteAduanal = value;
		}

		public function get mantenimiento():String
		{
			return _mantenimiento;
		}

		public function set mantenimiento(value:String):void
		{
			_mantenimiento = value;
		}

		public function get dificultad():String
		{
			return _dificultad;
		}

		public function set dificultad(value:String):void
		{
			_dificultad = value;
		}

		public function get extension1():String
		{
			return _extension1;
		}

		public function set extension1(value:String):void
		{
			_extension1 = value;
		}

		public function get extension2():String
		{
			return _extension2;
		}

		public function set extension2(value:String):void
		{
			_extension2 = value;
		}

		public function get esac():String
		{
			return _esac;
		}

		public function set esac(value:String):void
		{
			_esac = value;
		}

		public function get ev():String
		{
			return _ev;
		}

		public function set ev(value:String):void
		{
			_ev = value;
		}

		public function get vinculado():Boolean
		{
			return _vinculado;
		}

		public function set vinculado(value:Boolean):void
		{
			_vinculado = value;
		}

		public function get usuarioPConnect():Boolean
		{
			return _usuarioPConnect;
		}

		public function set usuarioPConnect(value:Boolean):void
		{
			_usuarioPConnect = value;
		}
	}
}