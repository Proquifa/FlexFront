package mx.com.proquifa.proquifanet.rsl.vista.vistas.comun.coverflowpqnet.clases
{
	...
	public class CoverFlowPQNetGris extends Group
	{
		private var _tweenDuration:int = 1200;
		private var _startTime:int;
		private var _elapsed:int;
		private var _iteration:uint
		private var _iterationCover:CoverComoUnGrupoGris;
		...
		private var _covers:Vector.<CoverComoUnGrupoGris>;
		...
		public function CoverFlowPQNetGris()
		{
			super();
		...
		public function load($familias:ArrayCollection,$mantenerFamiliaSeleccionada:Boolean = false):void
		{
			...
			_covers = new Vector.<CoverComoUnGrupoGris>;
			_coversContainer.removeAllElements();
			
			for (var i:int = 0; i < $familias.length; i++) 
			{
				var cover:CoverComoUnGrupoGris = new CoverComoUnGrupoGris();
				cover.id = i.toString();
		...
		private function unload():void
		{
			if(_covers && _covers.length > 0)
			{
				...
				{
					var cover:CoverComoUnGrupoGris = _covers[i];
		...
		private function ocultarBotonesDeLaFamilia($familia:int):void
		{
			if(_covers && _covers.length > 0)
			{
				var currentFamilia:CoverComoUnGrupoGris = _covers[$familia];
		...
		private function layoutCovers($actualizarInfo:Boolean = false):void
		{
			if(_covers && _covers.length > 0)
			{
				var currentCover:CoverComoUnGrupoGris;
		...
		private function determineLayout(destinationIndex:uint):void 
		{
			...
			if(_covers && _covers.length > 0 )
			{
				var iterationCover:CoverComoUnGrupoGris;
				var tempXPosition:Number;
				
				for (var i:uint = 0; i < _covers.length; i++) 
				{
					iterationCover = _covers[i] as CoverComoUnGrupoGris;
		...
		private function mostrarOpcionesEnLaFamilia():void
		{
			if(_covers && _covers.length > 0)
			{
				var currentFamilia:CoverComoUnGrupoGris = _covers[selectedIndex];
				//if(showIconos == true)
