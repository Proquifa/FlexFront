package mx.com.proquifa.proquifanet.rsl.vista.utils
{
	...
	public class Query
	{
		...
		private var _sinCache:Boolean = false;
		...
		public function getPunteros( $campos:Array, $omitirRepetidosEnCampo:String = "", $order:String = "" ):Array{
			...
			if( !sinCache && primaryKey != "" && _indexObject["cache-query"].hasOwnProperty( primaryKey ) ){
				// regresa el cache
				arrayReturn = _indexObject["cache-query"][primaryKey];
				return arrayReturn;
			}else{
		...
		public function getSumarCampo( $propiedad:String, $indices:Array, $omitirRepetidosEnCampo:String = "", $universo:Boolean=false ):Number{
			...
			if( !sinCache && _indexObject["cache-query"][primaryKey] != null && !$universo)
			{
		...
		[Bindable]
		public function get sinCache():Boolean
		{
			return _sinCache;
		}

		public function set sinCache(value:Boolean):void
		{
			_sinCache = value;
		}
