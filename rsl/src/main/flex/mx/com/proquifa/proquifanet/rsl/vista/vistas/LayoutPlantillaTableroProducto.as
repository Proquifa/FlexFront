package mx.com.proquifa.proquifanet.rsl.vista.vistas
{
	...
	public class LayoutPlantillaTableroProducto extends LayoutBase
	{
		public function LayoutPlantillaTableroProducto()
		{
			super();
		}
		...
		override public function updateDisplayList($containerWidth:Number, $containerHeight:Number):void{
			...
			var plantilla:PlantillaTableroProducto = (layoutTarget as PlantillaTableroProducto);
			var anchoVariableMenu:Number = anchoMenu;
			var xTabComparacion:Number;
			var tempWidth:Number;
			var xTabInfo:Number;
			var tempInfoWidth:Number;
						...
						switch(idElement)
						{
							...
							case "contenedorGraficas":
							{
								...
								if(plantilla.fxMenuResize != null && plantilla.fxMenuResize.isPlaying){
									...
									currentElement.setLayoutBoundsSize(currentElement.getLayoutBoundsWidth(),($containerHeight+((paddingTop + paddingBottom)*-1)));
									currentElement.setLayoutBoundsPosition(x,y);
									xTabInfo = currentElement.getLayoutBoundsWidth()+anchoVariableMenu + paddingLeft;
									tempInfoWidth = currentElement.getLayoutBoundsWidth();
								}else{
							...
							case "grpTabPanelInfo":
							{
								...
								if(plantilla.fxPanelInfoResize != null && plantilla.fxPanelInfoResize.isPlaying){
									if(plantilla.isMenuMinimixed)
										currentElement.setLayoutBoundsPosition(tempInfoWidth+paddingLeft,y);
									else
										currentElement.setLayoutBoundsPosition((tempInfoWidth + anchoMenu) + paddingLeft,y);
								}else{
									if(plantilla.isPanelInfoMaximixed)
							...
							case "grpPanelInfo":
							{
								if(plantilla.fxPanelInfoResize != null && plantilla.fxPanelInfoResize.isPlaying){
									if(plantilla.isMenuMinimixed){
										currentElement.setLayoutBoundsSize(($containerWidth+((paddingLeft+ paddingRight)*-1)) - tempInfoWidth,($containerHeight+((paddingTop+paddingBottom)*-1)));
										currentElement.setLayoutBoundsPosition(tempInfoWidth+paddingLeft,y);
									}else{
										currentElement.setLayoutBoundsSize(($containerWidth+((paddingLeft+ paddingRight)*-1)) - (tempInfoWidth + anchoMenu),($containerHeight+((paddingTop+paddingBottom)*-1)));
										currentElement.setLayoutBoundsPosition((tempInfoWidth + anchoMenu) + paddingLeft,y);
									}
								}else{
									if(plantilla.isPanelInfoMaximixed){
