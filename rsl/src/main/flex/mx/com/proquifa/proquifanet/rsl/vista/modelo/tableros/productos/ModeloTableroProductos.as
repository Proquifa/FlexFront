package mx.com.proquifa.proquifanet.rsl.vista.modelo.tableros.productos
{
	...
	public class ModeloTableroProductos extends EventDispatcher
	{
		...
		/**
		 * Obtener Compras Relizadas Grid
		 */
		private var _compraGrid:ArrayCollection;
		public function setComprasRealizadasGrid( $datos:ArrayCollection, $evento:EventoTableroProductos= null  ):void
		{
			_compraGrid = $datos;
			dispatchEvent( new Event("regresarComprasRealizadasGridModeloTabProductos") );
			
			if(_compraGrid!= null){
				if(actualizarEspera!= null && currentEnEspera){
					setControlDeBloqueoPantalla("terminaEspera",null,null,"regresarComprasRealizadasGridModeloTabProductos");
				}
			}
			if($evento != null && $evento.isIndividual){
				calcularLabelsIndividuales($evento);
		...
		[Bindable(event="regresarComprasRealizadasGridModeloTabProductos")]
		public function get enviarComprasRealizadasGrid():ArrayCollection{
			return _compraGrid;
		}
		...
		/**
		 * FUNCION RECIBE INFO PRODUCTO
		 */
		// etiqueta periodo
		private var informacionProducto:ConfiguracionPrecioProducto;
		public function setObtieneInformacionProducto($informacion:ConfiguracionPrecioProducto):void
		{
			if($informacion == null)
				return;
			
			informacionProducto = new ConfiguracionPrecioProducto();
			informacionProducto = $informacion;			
			
			dispatchEvent( new Event("obtieneInfoProductoEventoModeloTabProductos") );
		}
		
		[Bindable(event="obtieneInfoProductoEventoModeloTabProductos")]
		public function get obtenerInformacionProducto():ConfiguracionPrecioProducto
		{
