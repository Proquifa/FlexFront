package mx.com.proquifa.proquifanet.rsl.vista.modelo.reportes.compras.repProdAlmacen2012
{
	...
	public class ModeloRepProdAlmacen2012 extends EventDispatcher
	{
		...
		public function setProductoA($listaProductos:ArrayCollection):void{
			...
			/*var listPruebas:ArrayCollection = new ArrayCollection;
			var contador:int = 0;
			for each (var obj:* in $listaProductos) 
			{
				listPruebas.addItem(obj);
				if (++contador == 10)
					break;
			}
			
			$listaProductos = listPruebas;*/
		...
		public static var arrayCSV:ArrayCollection;
		public static var arrayCSVET:ArrayCollection;
		public static var arrayCSVFT:ArrayCollection;
		private function obtenerValoresPorClienteOProveedor($nombreClienYPro:Array, $enTiempo:String = null, $etiqutaParaBotonera:String = null):ArrayCollection{
			...
			var totalPiezas:uint = 0;
			var itemClienteOProvee:ReporteProductoAItemClientesProveedores;
			var itemClienteOProveeCSV:Object;
			var listaClientesProveedoresReturn:ArrayCollection = new ArrayCollection;
			
			if ($enTiempo == "ET" )
				arrayCSVET = new ArrayCollection;
			else if ($enTiempo == "FT")
				arrayCSVFT = new ArrayCollection;
			else
				arrayCSV = new ArrayCollection;
			...
			for (var i:int = 0; i < $nombreClienYPro.length; i++) 
			{
				...
				if($nombreClienYPro[i] != null){
					...
					if(punterosXNombre.length >0){
						...
						if ( (_currentSQL.universo[punterosXNombre[0]] as ProductoAlmacen).ruta_Zona == "Recoge en PROQUIFA") {
							itemClienteOProvee.ruta = "Recoge en PROQUIFA";
						} else {
						...
						itemClienteOProvee.rutaZona = (_currentSQL.universo[punterosXNombre[0]] as ProductoAlmacen).estadoProquifaNet;
						
						
						var punterosNormal:Array;
						var punterosNacional:Array;
						if(_isBusquedaAvanzada){
							if($etiqutaParaBotonera == null) {
								punterosNormal = _currentSQL.getPunteros([_currentDiaSeleccionado,_currentEtiqueta,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Normal",_proquifaNet,_esac,null,null,$enTiempo]);
								punterosNormal = punterosNormal.concat(_currentSQL.getPunteros([_currentDiaSeleccionado,_currentEtiqueta,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"null",_proquifaNet,_esac,null,null,$enTiempo]));
								
								punterosNacional = _currentSQL.getPunteros([_currentDiaSeleccionado,_currentEtiqueta,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Nacionales",_proquifaNet,_esac,null,null,$enTiempo]);
								punterosNacional = punterosNacional.concat(_currentSQL.getPunteros([_currentDiaSeleccionado,_currentEtiqueta,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Mundiales",_proquifaNet,_esac,null,null,$enTiempo]));
								punterosNacional = punterosNacional.concat(_currentSQL.getPunteros([_currentDiaSeleccionado,_currentEtiqueta,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Origen",_proquifaNet,_esac,null,null,$enTiempo]));
							}
							else {
								punterosNormal = _currentSQL.getPunteros([_currentDiaSeleccionado,$etiqutaParaBotonera,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Normal",_proquifaNet,_esac,null,null,$enTiempo]);
								punterosNormal = punterosNormal.concat(_currentSQL.getPunteros([_currentDiaSeleccionado,$etiqutaParaBotonera,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"null",_proquifaNet,_esac,null,null,$enTiempo]));
								
								punterosNacional = _currentSQL.getPunteros([_currentDiaSeleccionado,$etiqutaParaBotonera,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Nacionales",_proquifaNet,_esac,null,null,$enTiempo]);
								punterosNacional = punterosNacional.concat(_currentSQL.getPunteros([_currentDiaSeleccionado,$etiqutaParaBotonera,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Mundiales",_proquifaNet,_esac,null,null,$enTiempo]));
								punterosNacional = punterosNacional.concat(_currentSQL.getPunteros([_currentDiaSeleccionado,$etiqutaParaBotonera,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Origen",_proquifaNet,_esac,null,null,$enTiempo]));
							}
						}else{
							if($etiqutaParaBotonera == null) {
								punterosNormal = _currentSQL.getPunteros([_currentDiaSeleccionado,_currentEtiqueta,currentNombre,_manejoProducto,_tipoProducto,_marcaProducto,_tipoFlete,"Normal",_proquifaNet,_esac,null,null,$enTiempo]);
						...
						var enRuta:int = 0;
						var aEmbalar:int = 0;
						var porEmbalar:int = 0;
						var embalado:int = 0;
						var enInspeccion:int = 0;
						var porColectar:int = 0;
						var temp:ArrayCollection = new ArrayCollection;
						
						if (punterosNormal.length > 0 ) {
							temp.addItem(itemClienteOProvee);
							temp = ObjectUtil.copy(temp) as ArrayCollection;
							itemClienteOProveeCSV = temp.getItemAt(0) as Object;
							itemClienteOProveeCSV.control = "Normal";
							itemClienteOProveeCSV.monto = _currentSQL.getSumarCampo("pcompra.montoTotal",punterosNormal);
							itemClienteOProveeCSV.montoString = "$" + UtilsFormatosNumericos.precisionADosDecimalesConComas(itemClienteOProveeCSV.monto) + " USD";
							
							
							for (k = 0; k < punterosNormal.length; k++) {
								var estadoProquifa:String = (_currentSQL.universo[punterosNormal[k]] as ProductoAlmacen).estadoProquifaNet.toLowerCase();
								if ( estadoProquifa == "en ruta" || estadoProquifa == "en entrega" ){
									enRuta = 1;
									break;
								} else if (estadoProquifa == "embalado")
									embalado = 1;
								else if (estadoProquifa == "a embalar")
									aEmbalar = 1;
								else if (estadoProquifa == "por colectar")
									porColectar = 1;
								else if (estadoProquifa == "por embalar")
									porEmbalar = 1;
								else if (estadoProquifa == "en inspección")
									enInspeccion = 1;
							}
							
							if (enRuta == 1)
								itemClienteOProveeCSV.rutaZona = "En Ruta";
							else if (embalado == 1)
								itemClienteOProveeCSV.rutaZona = "Embalado";
							else if (aEmbalar == 1)
								itemClienteOProveeCSV.rutaZona = "A Embalar";
							else if (porColectar == 1)
								itemClienteOProveeCSV.rutaZona = "Por Colectar";
							else if (porEmbalar == 1)
								itemClienteOProveeCSV.rutaZona = "Por Embalar";
							else if (enInspeccion == 1)
								itemClienteOProveeCSV.rutaZona = "En Inspección";
							
							if ($enTiempo == "ET" )
								arrayCSVET.addItem(itemClienteOProveeCSV);
							else if ($enTiempo == "FT")
								arrayCSVFT.addItem(itemClienteOProveeCSV);
							else
								arrayCSV.addItem(itemClienteOProveeCSV);
						}
						
						temp = new ArrayCollection;
						if (punterosNacional.length > 0 ) {
							temp.addItem(itemClienteOProvee);
							temp = ObjectUtil.copy(temp) as ArrayCollection;
							itemClienteOProveeCSV = temp.getItemAt(0) as Object;
							itemClienteOProveeCSV.control = "Nacionales";
							itemClienteOProveeCSV.monto = _currentSQL.getSumarCampo("pcompra.montoTotal",punterosNacional);
							itemClienteOProveeCSV.montoString = "$" + UtilsFormatosNumericos.precisionADosDecimalesConComas(itemClienteOProveeCSV.monto) + " USD";
							
							for (k = 0; k < punterosNacional.length; k++) {
								var estadoProquifa:String = (_currentSQL.universo[punterosNacional[k]] as ProductoAlmacen).estadoProquifaNet.toLowerCase();
								if ( estadoProquifa == "en ruta" || estadoProquifa == "en entrega" ){
									enRuta = 1;
									break;
								} else if (estadoProquifa == "embalado")
			...
			if($cpedido != null){
				...
				for (var i:int = 0; i < punteros.length; i++){
					...
					if (($sql.universo[punteros[i]].estado == "STOCK" || $sql.universo[punteros[i]].estado == "Despachable" || $sql.universo[[punteros[i]]].estado == "A destrucción" ||
						...
						$sql.universo[punteros[i]].estado == "A surtir" || $sql.universo[[punteros[i]]].estado == "Por embalar" || $sql.universo[[punteros[i]]].estado == "A embalar" || $sql.universo[[punteros[i]]].estado == "A Embalar" || $sql.universo[[punteros[i]]].estado == "Por Colectar"
						|| $sql.universo[[punteros[i]]].estado == "Embalado" || $sql.universo[[punteros[i]]].estado == "En Mensajería" || $sql.universo[punteros[i]].estado == "En ruta") && 
