package mx.com.proquifa.proquifanet.rsl.vista.utils
{
	...
	public class UtilsFormatosNumericos
	{
		...
		public static function getMesesTrabajados(_anioFiltro:int=0,_anioFiscal:Boolean = false,_periodo:String = null):Number
		{
			...
			return _month;*/
			var porCalculo:Boolean;
			var fechaTmp:Date = new Date();
			
			if(_anioFiltro==0 && _anioFiscal == false && !_periodo){
				porCalculo=true;
			}else if(_anioFiscal == false && fechaTmp.getFullYear() == _anioFiltro){
				porCalculo=true;
			}else{
				porCalculo=false;
			}
			...
			if(porCalculo==true){
				...
				var numeroMes:Number;
				if(_anioFiscal)
					numeroMes = 12;
				else{
					if(_periodo.toLowerCase() == "anual")
						numeroMes = 12;
					else if(_periodo.toLowerCase() == "semestral")
						numeroMes = 6;
					else if(_periodo.toLowerCase() == "trimestral")
						numeroMes = 3;
					else if(_periodo.toLowerCase() == "mensual")
						numeroMes = 3;
				}
