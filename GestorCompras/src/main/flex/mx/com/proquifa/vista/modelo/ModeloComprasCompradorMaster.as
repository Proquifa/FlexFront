package mx.com.proquifa.vista.modelo
{
	...
	public class ModeloComprasCompradorMaster extends EventDispatcher
	{
		...
		private var listaDeclararArriboCD:ArrayCollection;
		...
		if(listaDeclararArriboCD != null && listaDeclararArriboCD.length != 0 )
			{
				for(var p:int=0; p < listaDeclararArriboCD.length; p++)
				{
					numeroPendientesTemporal.addItem(listaDeclararArriboCD[p]);
				}
		...
		public function setObtenerIndicadorPendienteDeclararArrivoCD(data:ArrayCollection):void
		{
			listaDeclararArriboCD = data;
