package mx.com.proquifa.vista.modelo.gestionarBackOrderXCancelar
{
	...
	public class ModeloGestionarBackOrderXCancelar extends EventDispatcher
	{
		...
		{   
			...
			for each (var provee:String in punteroProveedores ) 
			{
				...
				var punterosTempOc:Array;
				
				punterosTemp = query.getPunteros([provee]);
				punterosTempPro = query.getPunteros([provee],'idProducto');
				punterosTempOc = query.getPunteros([provee],'compra');
				tempReg.etiqueta = provee.toUpperCase();
				tempReg.totalProveedores = 1;
				tempReg.noProductos = punterosTempPro.length;
				/*tempReg.ocompras = query.getSumarCampo('totalOC',punterosTemp);*/
				/*tempReg.noProductos = query.getSumarCampo('totalProductos',punterosTemp);*/
				tempReg.ocompras = punterosTempOc.length;
			...
			for each (var origenA:String in punteroProveedores) 
			{
				...
				puntOrigenOC = query.getPunteros([null,null,null,null,origenA],'compra');
		...
		public function setObtenerRespuestaRegistrarBO(value:int):void
		{
			// 1 bien
			//-1 no bien
			...
			dispatchEvent(new Event("enviaRespuestaRegistrarBO"));
		}
		
		[Bindable(event="enviaRespuestaRegistrarBO")]
		public function get enviarRespuestaBO():int
		{
